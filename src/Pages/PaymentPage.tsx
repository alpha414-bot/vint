...
const PaymentPage = () => {
          ...
          if (["_c"].includes(key)) {
            return {
              [key]: deepClean({
                ...sr_value,
                ...{
                  reference: sr_value?.reference
                    ? `${(Math.random() + 8).toString(9)}`
                    : null,
                  tx_ref: sr_value?.tx_ref
                    ? `${(Math.random() + 8).toString(9)}`
                    : null,
                },
              }),
          ...
          if (["_m"].includes(key)) {
            return {
              [key]: deepClean({
                ...sr_value,
                ...{
                  accentColor: sr_value?.accentColor
                    ? sr_value.accentColor
                    : defaultAccentColor,
                  logoBgColor: sr_value?.logoBgColor
                    ? sr_value.logoBgColor
                    : "black",
                },
              }),
            };
  ...
  const pay = useCallback(() => {
    const handleSuccess = (ref: any) => {
      setLoadingState({
        status: true,
        text: "Please wait, Processing payment...<br/> Do not close window or reload..",
      });
      ...
      const downloadUrl = new URL(
        bonusCourse?.downloadable,
        window.location.origin
      );
      const supportLink = decodeURIComponent(data?._m?.support).replace(
        "{{message}}",
        encodeURIComponent(
          `Hello Support, I have successfully made payment for *${_.upperFirst(
            data?._m?.appName
          )} ${data?._m?.activePlanTitle}(NGN${
            data?._m?.amount
          })*. Here is my reference invoice number: ${ref} and my email is ${
            data?._m?.email
          }.`
        )
      );
      ...
      sendEmail(data?._m?.backend_email_url, {
        ...
        .catch((err) => {
          ...
          window.alert(
            "An error occurred while sending invoice. Please contact support."
          );
  ...
  const close = useCallback(() => {
    if (
      window.confirm("Are you sure you want to cancel the payment and go back?")
    ) {
      window.open(
        `${new URL("register", data?._m?.app?.domain).toString()}`,
        "_self"
      );
    }
  }, [data]);
  ...
  return (
    <MainLayout
      ...
      no_navbar
      no_footer
      accent_color={data?._m?.accentColor}
    >
      <div
        className={classNames(
          "p-4 md:p-6 lg:p-12 relative w-full h-scren overflow-hidden items-center justify-center flex flex-col gap-6 bg-gray-100",
          {
            "h-screen": !DEBUG,
          }
        )}
      >
        {paymentStatus.status === "pending" ? (
          <div className="w-full max-w-md space-y-4 mx-auto">
            <button
              ...
              className="p-0.5 rounded-full bg-white/80 hover:bg-white shadow-md inline-flex items-center justify-center mb-2 border-2"
              style={{ borderColor: data?._m?.accentColor }}
            >
              <svg
                className="w-8 h-8"
                aria-hidden="true"
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p>
                  Pay securely on the web for {_.upperFirst(data?._m?.appName)}.{" "}
                </p>
              </div>
              <img
                src={data?._m?.app?.logo}
                className={classNames(
                  "w-12 h-12 object-contain object-center rounded-full p-0.5"
                )}
                style={{ backgroundColor: data?._m?.logoBgColor }}
              />
            </div>
            <input
              type="text"
              placeholder="Enter Amount"
              className="input-checkout !text-gray-800 font-bold"
              value={`${data?._m?.activePlanTitle} for ${_.upperFirst(
                data?._m?.appName
              )} @ ${price(data?._m?.amount)}`}
              disabled
              name="amount"
            />
            <input
            ...
            <div className="text-base lg:text-lg font-medium">
              <span>
                If You Need more Explanation on{" "}
                {_.upperFirst(data?._m?.appName)}, or You have Any Issue with
                Making Payment, contact support by clicking on the below button
              </span>
              <button className="inline-flex gap-0.5 items-center bg-red-500/35 underline underline-offset-2 text-sm py-1 text-gray-800 font-semibold px-2 rounded-md">
                {" "}
                <svg
                  className="w-6 h-6"
                  aria-hidden="true"
            ...
            <div className="flex justify-end mt-8">
              <button
                ...
                className="text-sm py-2 text-white px-4 rounded-md flex items-center gap-1"
                style={{ backgroundColor: data?._m?.accentColor }}
              >
                <svg
                  className="w-6 h-6 "
                  aria-hidden="true"
          ...
          <div className="text-center space-y-4 w-full max-w-md mx-auto">
            ...
            <p className="text-lg">
              Thank you for your payment. An email confirmation has been sent to
              your registered email address containing instructions and your
              coupon code. Your transaction reference is:
            </p>
            <div className="relative">
              <pre className="bg-gray-200 p-4 rounded-md text-left break-all">
            ...
            <p className="text-lg">
              Click on the button below to contact support in order to use or
              get your coupon code. You would redirected to this page in the
              next {redirectCountdown} seconds.
            </p>
            <button
              onClick={() => {
                const supportLink = decodeURIComponent(
                  data?._m?.support
                ).replace(
                  "{{message}}",
                  encodeURIComponent(
                    `Hello Support, I have successfully made payment for *${_.upperFirst(
                      ...
                      data?._m?.amount
                    })*. Here is my reference invoice number: ${
                      paymentStatus.reference
        ...
        {loading.status && (
          <div className="fixed inset-0 z-50 bg-gray-900/70 flex items-center justify-center">
            <Spinner
              className="w-12 h-12"
              accentColor={data?._m?.accentColor}
              text={loading.text}
            />
          </div>
        )}
